...
- **`ops/feishu-to-xianyu.mjs` 丝滑化（2026-07-28）**：submit 前只读一次 live，默认从 01–04 动态选择可跑目标；目标 lease/offline/quarantine 永远硬拦，unresolvedFailure 要先恢复，`--force` 收窄为 `FORCE=ready-only`。飞书商品目录表 `tblUcslarq5iLEPB` 按 SKU+`READY_TO_PUBLISH` 取一条 → 本地下图 → 组装 `xianyu.publish.full_dry_run` fixture → submit+poll。`--dry-run` 现在只规划 phonePath，**零手机写入**；真要只推图用显式 `--prep/--push-only` 后退出。推图仍是编排层已知 gap，生产验收继续只认 devicectl job/session + 可见 lease。
...
- **dry-run 实证（2026-07-28）**：`--sku DX1488-100 --aliases 02 --actor codex-silk-p5 --dry-run` 读取真实飞书记录、3 张图仅下载到 Mac、生成 `/tmp/f2x-phase5-dryrun/fixture-02-full.json`，日志明确未调用 phone-push；fixture 为单色只含尺码 S/M/L/XL/XXL，`saveDraft=false`。A 仓 `npm test` 22/22、`npm run check` 全绿。
