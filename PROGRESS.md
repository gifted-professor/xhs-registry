...
- **registry 仓改动**：`ops/_xhs-parse.mjs` `findFollowBtn` 收紧为 **exact-set 等值** `{关注,已关注,回关,相互关注}`（trim，text/desc 分开，避免 `关注的话题` 假阳）；`followState` 修正 `回关`→unfollowed（对方关注你、你未关注，tap 即回关；旧代码误归 followed 会被 Explorer 当已关注跳过）；新增 `findProfileAuthor(xml)`（tier-1 `^头像[,，]` desc 取名，tier-2 顶部 TextView 兜底仅诊断）；新增 `findProfileFollowBtn(xml)`（**overlay 消歧**——同屏多精确 `关注` label 时通用 findFollowBtn first-match 命中背景节点 y=161，本函数仅在 tier-1 头像存在时取头像下方、含于最小面积 enabled clickable 宽容器（≥屏宽 30%）的 CTA，零/多候选 fail-closed；round-3 加固四项：屏宽取「可信 root」= 包含头像的节点里面积最大者（全屏窗口，稀疏/截断 dump 无则 fail-closed，非首个解析节点）、排除全屏/近全屏（宽≥屏宽 90%）clickable 消散层、text/desc 冲突时 matched 取四态字段（非 text||desc）、同面积并列最小容器全收交唯一性判断不靠输入序取首个）。新建 `tests/xhs-parse.test.mjs`（20 fixture：四状态/desc-only/缺失/假阳/作者正确/缺失/错误目标 + overlay 消歧 13 例：选宽 CTA(254,998)/四态保持/无头像非浮层/零候选/双候选/无容器/同中心异 bounds 双容器/离屏大 R 不抬阈值 + round-3 缺可信 root 稀疏 dump/截断子屏首节点/全屏 wrapper 排除/text-desc 冲突取四态字段/同面积异 bounds 最小容器不唯一 fail-closed）。
...
- **待办（诚实）**：
  ...
  - **`头像,<name>` content-desc 格式已实证**（2026-07-29 Explorer 在 01 抓真实主页浮层 dump `overlay-01.xml` 确认：`ImageView clickable cy=364 content-desc="头像,<nickname>"` 半角逗号，`findProfileAuthor` tier-1 命中）；**overlay 关注按钮定位 bug 已修**（`findProfileFollowBtn` 取宽 CTA 容器 (254,998)，旧 `findFollowBtn` first-match 误中 y=161 背景节点）——**Risk #1 解析侧已闭**（registry parser 实现完成，Engineer 自测与离线 replay 通过，等待 Hermes 独立验证；round-3 已补修 Hermes 第二轮只读复验的 4 项 supplemental findings：缺可信 root 误点统计 / 全屏 wrapper 当 CTA / text-desc 冲突 matched 取错 / 同面积最小容器歧义，各加回归测试，parse 20/20 绿且真实 overlay replay 仍 (254,998) 无回归，仍待 Hermes 复验），剩 GPFS `fast-operator.mjs` port 同选择器 + 部署 + Hermes 真机回归。
...
14. **xhs.follow.ensure 待部署 + 待实证（2026-07-29）**：capability 代码+测试已就绪（GPFS 233/233、registry 46/46）但**未部署 Windows**；`头像,<name>` 头像 content-desc 格式**已真实 dump 实证**（overlay-01.xml），overlay 关注按钮定位 bug 已修（`findProfileFollowBtn`）；registry parser 实现完成、Engineer 自测与离线 replay 通过，等待 Hermes 独立验证（round-3 补修 Hermes 第二轮复验的 4 项 supplemental findings，parse 20/20 绿 + 真实 overlay replay 仍 (254,998) 无回归）——剩 GPFS operator port 同选择器 + 部署 + Hermes 10+ 真机回归（req#12，独立验收，未执行）。详见上文 `xhs.follow.ensure capability` 节。
