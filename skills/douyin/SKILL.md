...
|-------|--------|--------|--------|------|------|
| 01 | `1511f78c` | **已登录** | 正常（推荐 Feed / 右侧栏可用） | ✅ 默认 | 探索主机；偶发 dump 抖，force-stop 可恢复 |
| 02 | `9b18cccb` | **已登录** | 正常 | ✅ 默认 | 曾弹青少年模式「我知道了」，过引导后 Feed 可用 |
| 03 | `211d0120` | 未核 | `ready!=true` / 不在 17910 | ❌ | preflight 失败，暂不入集合 |
| 04 | `H6NNHU8LLFHAIRLV` | **未登录** | Feed/右侧栏不可用（`dump_feed`/空层） | ❌ | **根因是未登录**，勿写成「青少年模式」或单纯 a11y；登录后再纳入 |
